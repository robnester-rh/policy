...
*Solution*: Make sure to configure either a signing identity under the `rh-release` key in the `signing_identities` xref:signing_identities.adoc#_signing_identities_structure[configuration] or a list of trusted registry prefixes in `allowed_registry_prefixes`.
