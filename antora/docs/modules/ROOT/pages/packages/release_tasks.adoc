...
Ensure that every currently required test task is included in either the build PipelineRun or the latest signature-verified ITS PipelineRun for an integration test associated with the image.
...
Produce a warning when a test task that will be required in the future was not included in either the build or latest applicable ITS PipelineRun attestations.
