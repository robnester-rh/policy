...
* https://github.com/conforma/policy/blob/{page-origin-refhash}/policy/release/tasks/tasks.rego#L287[Source, window="_blank"]
...
* https://github.com/conforma/policy/blob/{page-origin-refhash}/policy/release/tasks/tasks.rego#L197[Source, window="_blank"]

[#tasks__required_untrusted_test_task_found]
=== link:#tasks__required_untrusted_test_task_found[All required test tasks are from trusted tasks]

Ensure that every required test task found in a build or latest applicable ITS PipelineRun has a trusted execution path. For an ITS PipelineRun, every applicable Task represented in its SLSA provenance must be trusted.

*Solution*: Ensure the required test task runs in a PipelineRun whose applicable Tasks all satisfy the configured trusted task rules.

* Rule type: [rule-type-indicator failure]#FAILURE#
* FAILURE message: `%s is required and present, but PipelineTask %q (Task %q) in its provenance is untrusted`
...
[#tasks__required_test_tasks_found]
=== link:#tasks__required_test_tasks_found[All required test tasks were included in a pipeline]

Ensure that every currently required test task is included in either the build PipelineRun or the latest task-trusted ITS PipelineRun for an integration test associated with the image.

*Solution*: Ensure the required test task runs in either the build pipeline or an Integration Test Service pipeline associated with the image.

* Rule type: [rule-type-indicator failure]#FAILURE#
* FAILURE message: `%s is missing`
* Code: `tasks.required_test_tasks_found`
* Effective from: `2027-01-15T00:00:00Z`
* https://github.com/conforma/policy/blob/{page-origin-refhash}/policy/release/tasks/tasks.rego#L226[Source, window="_blank"]

[#tasks__data_provided]
=== link:#tasks__data_provided[Data provided]

Confirm the expected data keys have been provided in the expected format. The keys are `pipeline-required-tasks`, `required-tasks`, and `required-test-tasks`.
...
* https://github.com/conforma/policy/blob/{page-origin-refhash}/policy/release/tasks/tasks.rego#L418[Source, window="_blank"]
...
* https://github.com/conforma/policy/blob/{page-origin-refhash}/policy/release/tasks/tasks.rego#L78[Source, window="_blank"]

[#tasks__future_required_test_tasks_found]
=== link:#tasks__future_required_test_tasks_found[Future required test tasks were found]

Produce a warning when a test task that will be required in the future was not included in either the build or latest task-trusted ITS PipelineRun attestations.

*Solution*: Ensure the test task runs in either the build pipeline or an Integration Test Service pipeline associated with the image.

* Rule type: [rule-type-indicator warning]#WARNING#
* WARNING message: `%s is missing and will be required on %s`
* Code: `tasks.future_required_test_tasks_found`
* Effective from: `2027-01-16T00:00:00Z`
* https://github.com/conforma/policy/blob/{page-origin-refhash}/policy/release/tasks/tasks.rego#L108[Source, window="_blank"]
...
* https://github.com/conforma/policy/blob/{page-origin-refhash}/policy/release/tasks/tasks.rego#L350[Source, window="_blank"]
...
* https://github.com/conforma/policy/blob/{page-origin-refhash}/policy/release/tasks/tasks.rego#L56[Source, window="_blank"]
...
* https://github.com/conforma/policy/blob/{page-origin-refhash}/policy/release/tasks/tasks.rego#L325[Source, window="_blank"]
...
* https://github.com/conforma/policy/blob/{page-origin-refhash}/policy/release/tasks/tasks.rego#L166[Source, window="_blank"]
...
* https://github.com/conforma/policy/blob/{page-origin-refhash}/policy/release/tasks/tasks.rego#L378[Source, window="_blank"]
